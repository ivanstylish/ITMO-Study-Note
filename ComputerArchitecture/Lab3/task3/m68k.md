...
- **Status Register (SR):** Contains condition codes (flags):
    - **N (Negative):** Set if the result of an operation is negative. Установите значение, если результат операции отрицательный.
    - **Z (Zero):** Set if the result of an operation is zero. Устанавливается, если результат операции равен нулю.
    - **V (Overflow):** Set if an arithmetic overflow occurs. Устанавливается в случае арифметического переполнения.
    - **C (Carry):** Set if a carry or borrow occurs in arithmetic operations. Укажите, если в арифметических операциях происходит перенос или заимствование.
