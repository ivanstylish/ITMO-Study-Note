...
GET-запрос — это стандартный метод протокола HTTP, используемый клиентом (например, браузером) для запроса и получения данных с веб-сервера без изменения его состояния. Все параметры такого запроса добавляются к URL-адресу в виде "параметров строки запроса" (после знака ? и разделяясь &)  
GET 请求是客户端（例如浏览器）使用的一种标准 HTTP 协议方法，用于在不更改其状态的情况下从 Web 服务器请求和接收数据。
Запрос ресурса: Когда вы вводите адрес сайта в браузере или нажимаете ссылку, браузер отправляет HTTP-запрос с методом GET на сервер.  
Запрос ресурса: 当你在浏览器中输入网站地址或点击链接时，浏览器会向服务器发送 HTTP 请求，请求方法为 GET。  
...
Основные теги в HTML используются для структурирования контента (например, <html>, <body>, <p>, <h1> для заголовков), а атрибуты предоставляют дополнительную информацию или модифицируют поведение тега (например, href для ссылок <a>, src и alt для изображений <img>, а также глобальные атрибуты class, id, style и title для всех элементов). 
...
"Div" может означать несколько вещей, но чаще всего это тег <div> в HTML — блочный контейнер для группировки элементов и создания макетов веб-страниц, позволяющий стилизовать содержимое с помощью CSS.  
HTML 中的 <div> 标签是用于对元素进行分组和创建网页布局的块容器，允许您使用 CSS 设置内容样式 。  

3. Структура HTML-страницы. Объектная модель документа (DOM).
HTML 页面的结构。文档对象模型 （DOM）。  

Структура HTML-страницы состоит из объявления типа документа (<!DOCTYPE html>), корневого элемента <html>, содержащего <head> (метаинформацию о странице, например, заголовок и кодировку) и <body> (видимое содержимое страницы, такое как текст, изображения и ссылки). Элементы <head> и <body> являются основными блоками, а всё содержимое страницы находится внутри тега <body>.  
HTML 页面的结构由文档类型声明 （<！DOCTYPE html>）， 根元素 <html>，其中包含 <head>（有关页面的元信息，例如标题和编码）和 <body>（可见页面内容，例如文本、图像和链接）。 <head> 和 <body> 元素是主块，所有页面内容都在 <body> 标记内。   
...
ES7
Добавлена операция возведения в степень (**), Array.prototype.includes().Используется для определения, содержит ли массив значение
...
3. Буква "d" в названиях серверных демонов, таких как httpd, ftpd, sshd, означает "daemon".

Демон — это фоновый процесс в операционных системах семейства Unix/Linux, который не взаимодействует напрямую с пользователем и выполняет определенные системные функции.

httpd — это HTTP-демон. По сути, это сама программа веб-сервера (например, Apache HTTP Server), которая постоянно работает в фоне, ожидая входящие HTTP-запросы на определенном порту (обычно 80 или 443), и обрабатывает их.

Таким образом, httpd = HTTP Daemon.

4. Коды ошибок HTTP
HTTP коды состояния — это трехзначные， которые сервер отправляет клиенту (например, браузеру) в ответ на его запрос. Они сообщают о результате обработки запроса. Коды разделены на пять классов:
...
200 OK — стандартный ответ для успешного HTTP-запроса.

201 Created — запрос привел к созданию нового ресурса.

3xx: Перенаправление

Для завершения запроса требуются дальнейшие действия.

Примеры:

301 Moved Permanently — ресурс был permanently перемещен на новый URL.

302 Found — ресурс временно доступен по другому URL.

4xx: Ошибка клиента

Ошибка на стороне клиента (например, неверный запрос).
...
Сервер не смог выполнить допустимый запрос.

Примеры:

500 Internal Server Error — общая ошибка сервера, когда он столкнулся с непредвиденной ситуацией.
