...
- CGI каждый раз создает новый экземпляр процесса для обслуживания запроса. 
...
Model 1
Принцип: Логика приложения и представление тесно связаны и объединены в одном или нескольких JSP-файлах.  

Реализация на Java EE:  
Клиент: Браузер отправляет запрос на сервер.  
Сервер: Сервлет (или JSP-страница) получает запрос, выполняет всю бизнес-логику (запросы к базе данных, вычисления) и затем напрямую генерирует HTML-ответ, который отправляется клиенту.  

Недостатки:  
Сложность поддержки: Бизнес-логика и логика представления перемешаны, что затрудняет внесение изменений и поддержку кода.  
Масштабируемость: Низкая масштабируемость, так как нет четкого разделения ответственности между компонентами.   

Model 2  
Принцип: Используется шаблон Model-View-Controller (MVC), который разделяет приложение на три четко определенные роли.
Реализация на Java EE:  
Клиент: Браузер отправляет запрос на сервер.  
Контроллер: Java-сервлет (или другая реализация контроллера) получает запрос. Он обрабатывает запрос, выбирает нужную модель данных и передает управление представлению.  
Модель: Содержит данные и бизнес-логику. Контроллер взаимодействует с моделью, а модель возвращает данные для представления.  
Представление: JSP-страница получает данные от контроллера и генерирует HTML-ответ для клиента.  
Преимущества:  
Разделение ответственности: Четкое разделение логики приложения на модель, представление и контроллер.  
Гибкость: Возможность изменять каждую часть приложения независимо, например, можно легко заменить JSP-страницу, не меняя бизнес-логику в модели.  
Масштабируемость: Улучшенная масштабируемость за счет четкой структуры и разделения ответственности.   