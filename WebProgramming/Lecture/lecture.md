...
#### Проявления несоответствия 不合规的表现
...
有三种可能的方案：  
1) 不带身份管理的标准存储层（无身份范围）。
2) 具有上下文范围身份的存储层。
3) 具有严格身份管理的存储层（进程范围身份）。

#### Наследование и полиморфизм в ORM 继承和多态的表示
...
在对象关系映射（ORM）中，继承和多态密切相关。  
ORM 中有三种实现继承的方式。  

#### 如何在java上实现ORM
- JDBC
- ORM-framework(Hibernate, EclipseLink, TopLink..)
...
Red Hat 的 ORM 框架，自 2001 年以来一直在开发。
主要特性：
- 数据库表可以用 XML 文件或注解进行描述。
- 提供两种查询编写方式：HQL 和 Criteria API。
- 支持编写原生 SQL 查询。
- 可与 Apache Lucene 集成，实现全文数据库搜索（Hibernate Search）。
...
Eclipse 基金会开发的 ORM 框架。  
主要特性：
- 基于 Oracle TopLink 代码库。
- 是 JPA 的参考实现。
...
Java 标准（JSR 220、JSR 317）定义了：  
- Java 对象在数据库中的存储方式；
- 用于操作已存储 Java 对象的 API；
- 查询语言（JPQL）；
- 在各种环境下的可用性。
...
#### Взаимодействие приложения с БД через JPA 应用程序通过 JPA 与数据库交互
![](../pic/26.png)  

Java实体类和数据库表之间的映射关系过程：  
![](../pic/27.png)  
...
- 支持静态查询和动态查询。
- 查询语句可以使用 SQL 或 JPQL 编写。
- 支持传递命名参数和位置参数。
- 使用 fetch 时支持立即访问。

#### JPA Queries API
Для создания запроса:  
- `EntityManager.createQuery()` - создает пустой Query.
- `EntityManager.createQuery(String jpql)` - создает Query на основе JPQL-строки.
- `EntityManager.createNamedQuery(String name)` - создает Query на основе именованного запроса.
- `createNativeQuery()` - создает Query на основе SQL-запроса.

Для получения результата:
- `Query.getResultList()` - возвращает список объектов.
- `Query.getSingleResult()` - возвращает один объект.
