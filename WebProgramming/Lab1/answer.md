...
HTTP（超文本传输​​协议）是一种应用层数据传输协议（最初以 HTML 格式的超文本文档形式出现，目前用于传输任意数据）。HTTP 基于客户端-服务器技术，这意味着它假设存在：  
...
脚本 - 网页中包含的代码。客户端脚本在客户端计算机上执行，因此需要内置解释器。使用 <script> 标签可以脚本插入网页。
...
特别需要补充的是，代码块不会限制函数的作用域。如果在函数外部声明变量，则该变量属于全局作用域。函数本身也是一个对象。
...
CGI 是一种过时的技术，它允许 Web 服务器与应用服务器交互。对于每个请求，都会启动一个带有 PHP 解释器的进程，并在返回响应后终止该进程。由于这种机制效率低下，因此 FastCGI 应运而生，它的解释器进程不会被终止，而是用于后续请求。  
...
PHP 通过 pthreads 扩展具有多线程功能，但在使用 PHP 作为应用程序服务器时不可用，仅在编写命令行脚本时可用。
