...
代码实现框架：
客户端：
1. network包下的UDPClient类，用于发送数据到服务器端，并接收服务器端的响应。
2. controllers包下的各个类分别用于，登录注册页面的形成和处理，以及主页面的形成和功能，其中也包括了各个命令的处理。
3. auth包下的类用于，获取页面的第一语言选择（英语）和用户信息
4. ui中的对话管理器，用于管理各个对话框的显示和隐藏以及对于正确或错误信息的提示。
5. utility包下的localizator用于读取本地化的语言资源文件，并根据用户的选择来显示相应的语言。
   
服务端：
1. network包下的UDPServer类，用于接收客户端的请求并返回响应。UDPDatagramChannel类用于处理数据包的接收和发送。
2. handlers包下的各个命令类用于处理客户端的请求，包括登录注册页面的处理，以及主页面各个命令的功能。