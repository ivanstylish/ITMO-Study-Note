import numpy as np
import matplotlib
# 解决 PyCharm SciView 的 Bug，强制使用非交互式后端保存图片，而不弹窗
matplotlib.use('Agg') 
...
class RBFNetwork:
    ...
    def _kmeans_from_scratch(self, X, max_iter=100):
        ...
        # Выбираем крайние точки как начальные центры для стабильности
    ...
    def fit(self, X, y, epochs=5000, lr=0.005):
        ...
        for epoch in range(epochs):
            ...
            # Подробная запись каждые 1000 итераций и на 1-й итерации
            if epoch % 1000 == 0 or epoch == 0:
                print(f"Эпоха {epoch}: Loss = {mse:.6f} | Веса = {self.weights} | Сигмы^2 = {self.sigmas_sq}")
            
            # Вычисление градиентов (ИСПРАВЛЕНО: убраны лишние минусы)
            ...
            for j in range(self.n_neurons):
                ...
                grad_sigmas_sq[j] = np.mean(errors * self.weights[j+1] * phi[:, j] * r_sq) / (2 * self.sigmas_sq[j]**2)
            
            # Обновление параметров (Градиентный спуск: вычитаем градиент)
...
loss_history = rbf.fit(X_data, y_data, epochs=5000, lr=0.005)
...
s1, s2 = rbf.sigmas_sq
print(f"\nАналитический вид модели:")
print(f"z(x,y) = {w0:.4f} + {w1:.4f} * exp(-((x-{cx1:.4f})^2+(y-{cy1:.4f})^2)/(2*{s1:.4f})) + {w2:.4f} * exp(-((x-{cx2:.4f})^2+(y-{cy2:.4f})^2)/(2*{s2:.4f}))")
...
ax1.set_xlabel('Итерация'); ax1.set_ylabel('MSE Loss'); ax1.grid(True)

# 2. 3D поверхность
x_grid = np.linspace(0, 6, 50); y_grid = np.linspace(0, 6, 50)
...
ax3.set_title('Линии уровня'); plt.colorbar(contour, ax=ax3)

plt.tight_layout()
# 解决报错：直接保存为图片，不调用 plt.show()
plt.savefig('rbf_network_plot.png', dpi=300)
print("\n✅ Готово! График сохранен в файл 'rbf_network_plot.png'")