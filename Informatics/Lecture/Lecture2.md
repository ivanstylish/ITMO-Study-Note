...
- Big-endian: старший значащий байт хранится по низкому адресу, в основном используется в сетевой связи и некоторых системах RISC.最高有效字节在低地址，主要用于网络通信和部分 RISC 系统。Порядок с прямым порядком байтов является доминирующим порядком в сетевых протоколах.大端顺序是网络协议中的主要顺序。
