...
### 变体3答案
1. Предположим ИС для сказки: Колобок «убегает» от персонажей, система управляет встречами. 假设有一个童话故事的IS：Kolobok“逃离”了故事中的人物，系统负责管理他们的会面。

**Модель требований:**

FR0: «ИС должна регистрировать встречи Колобка с персонажами»
FR1: «ИС должна определять исход встречи (убежал / съеден)»
FR2: «ИС ведёт историю маршрута»

**Use-Case:**
Акторы: Колобок, Персонаж (Заяц/Волк/Медведь/Лиса), Сказочник (администратор).  
Прецеденты: «Начать маршрут»,   
«Зарегистрировать встречу»,   
«Определить исход»,   
«Завершить маршрут».  

**Доменная модель:**
Колобок (статус: {вкатился, убежал, съеден}),  
Персонаж (имя, тип),  
Встреча (место,  
результат: {убежал, съеден}),  
Маршрут (список встреч, итог).  
...
- Inception — Начальная стадия (Фаза начала)

- Elaboration — Стадия уточнения (Проектирование)

- Construction — Стадия конструирования (Разработка)

- Transition — Стадия внедрения (Переход)
...
1. Ресурсные, бизнес, технические, политические риски для ИС «ТТБ» + Экспозиция
![](image-1.png)  
...
- Предоставляет гибкость и адаптивность для различных типов проектов и 
организаций 
- Хорошо подходит для больших проектов. 
- Учитывает весь жизненный цикл проекта, включая планирование, архитектуру, 
тестирование и развертывание. 
- Позволяет совмещать Agile и не-Agile подходы в зависимости от требований 
проекта. 
...
- У DAD есть высокий уровень сложности из-за большого количества различных 
процессов, практик и инструментов. 由于涉及大量不同的流程、实践和工具，DAD 具有很高的复杂性。
- Эта модель требует высокого уровня компетенции команды для правильной 
реализации всех процессов и практик. 该模式要求团队具备较高的能力，才能正确实施所有流程和实践。
