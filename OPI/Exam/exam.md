...
Диаграмма состояний UML (State Machine Diagram) — это поведенческая диаграмма, которая показывает, как объект или система меняет своё состояние под воздействием событий.
Основные элементы:
- Состояние (State) — состояние объекта в определённый момент. Изображается прямоугольником с закруглёнными углами.
- Начальное состояние (Initial State) — точка начала работы, обозначается закрашенным кружком ●.
- Конечное состояние (Final State) — завершение работы, обозначается ◉.
- Переход (Transition) — стрелка между состояниями.
- Событие (Event) — событие, вызывающее переход.
- Условие (Guard condition) — дополнительное условие перехода, обычно записывается в [ ].
- Действие (Action) — операция, выполняемая при переходе или в состоянии.
Обычно переход записывается так:
событие [условие] / действие
Например, диаграмма состояния заказа:
● → Создан → Оплачен → Отправлен → Доставлен → ◉
При этом переход Создан → Оплачен происходит после события оплата, а Оплачен → Отправлен — после события отправка товара.

UML 状态图（State Machine Diagram）是一种行为图，用来描述对象或系统在事件作用下如何从一个状态转换到另一个状态，也就是对象的生命周期。
主要元素包括：
- 状态（State）：对象当前所处的状态，通常使用圆角矩形表示。
- 初始状态（Initial State）：状态机开始的位置，用实心圆 ● 表示。
- 终止状态（Final State）：状态机结束的位置，一般用 ◉ 表示。
- 转换（Transition）：状态之间的箭头。
- 事件（Event）：触发状态转换的事件。
- 守卫条件（Guard condition）：转换必须满足的条件，一般写成 [条件]。
- 动作（Action）：转换发生时或者状态内部执行的操作。
转换通常可以写成：
事件 [条件] / 动作
例如一个订单系统：
● → 已创建 → 已付款 → 已发货 → 已送达 → ◉
其中，“付款”事件使订单从“已创建”转换为“已付款”，“发货”事件使其从“已付款”转换为“已发货”。
...
UP — Unified Process — это итеративная методология разработки ПО, сформировавшаяся в 1990-х годах на основе объектно-ориентированного подхода и UML.
Главные идеи UP:
- разработка ведётся итерациями и инкрементами;
- процесс ориентирован на варианты использования (use cases);
- большое внимание уделяется архитектуре системы;
- риски стараются выявлять и устранять как можно раньше.
Обычно выделяют 4 фазы:
1. Inception — Начало: определяются цели проекта, основные требования и границы системы.
2. Elaboration — Уточнение: уточняются требования, создаётся базовая архитектура, анализируются основные риски.
3. Construction — Построение: основная разработка, реализация и тестирование системы.
4. Transition — Передача: внедрение продукта, исправление ошибок, обучение пользователей.
RUP — Rational Unified Process — это конкретная коммерческая реализация Unified Process, разработанная компанией Rational, позже IBM.
Основы RUP:
- итеративная разработка;
- управление требованиями;
- use-case driven подход;
- architecture-centric подход;
- постоянное тестирование качества;
- управление изменениями и конфигурациями.
Важно: каждая итерация обычно включает несколько видов деятельности: анализ требований, проектирование, реализацию, тестирование и развёртывание.

UP（Unified Process，统一过程） 是 1990 年代形成的一种软件开发方法，主要基于面向对象思想和 UML。
它的核心特点是：
- 迭代式和增量式开发；
- 以用例（Use Case）为驱动；
- 以系统架构为核心；
- 尽早发现并降低项目风险。
UP 通常分为四个阶段：
1. Inception，初始阶段：确定项目目标、范围和主要需求。
2. Elaboration，细化阶段：进一步明确需求，建立核心架构，解决主要风险。
3. Construction，构建阶段：进行主要编码、实现和测试。
4. Transition，交付阶段：部署系统、修复问题，并交付给用户。
RUP（Rational Unified Process） 是 Rational 公司提出的 UP 的一种具体、更加规范化的实现，后来归入 IBM。
RUP 的基础包括：
- 迭代开发；
- 需求管理；
- 用例驱动；
...
Gradle — это современная система автоматической сборки с открытым исходным кодом, основанная на концепциях Apache Ant и Apache Maven. В отличие от XML-формата Maven, Gradle использует предметно-ориентированный язык (DSL) на основе Groovy или Kotlin для написания сценариев сборки.  
Gradle 是一个基于 Apache Ant 和 Apache Maven 概念的开源构建自动化系统。它使用一种基于 Groovy 或 Kotlin 的领域特定语言（DSL）来编写构建脚本，而不是 Maven 的 XML 格式。  

**Преимущества Gradle**
- Высокая производительность
- Гибкость
- Лаконичный и мощный DSL 简洁且强大的 DSL
- Управление зависимостями 卓越的依赖管理
- Поддержка многопроектных сборок: Эффективно управляет зависимостями и порядком сборки в проектах с множеством модулей. 多项目构建支持

**Файл сборки**
Файлы сборки Gradle существуют в двух форматах:
- `build.gradle` — на основе DSL Groovy (классический, динамический).
- `build.gradle.kts` — на основе DSL Kotlin (рекомендуемый официально, обеспечивает лучшую автодополнение и проверку типов в IntelliJ IDEA).
...
1. Полное тестовое покрытие 完整测试覆盖度
Это степень выполнения кода тестами. Основные метрики: покрытие операторов, ветвей, условий, путей, требований. 指测试执行时覆盖代码的程度，常用指标：语句覆盖、分支覆盖、条件覆盖、路径覆盖、需求覆盖

2. Достижимость

- Теоретически: для простых программ без циклов 100% покрытие путей достижимо.理论：对无循环的小程序，100%路径覆盖可行。
- Практически: в больших системах невозможно из-за взрыва путей, недостижимого кода и внешних зависимостей. 实际：大型系统几乎不可能，原因包括：路径组合爆炸、不可达代码、外部环境依赖。
- Важно: 100% покрытие не гарантирует отсутствие ошибок. Нужен риск-ориентированный подход. 结论：100%覆盖≠无缺陷，应风险驱动，优先核心模块。

3. Пример
Функция классификации возраста (Child/Adult/Senior/Invalid).

Граничные значения (-5, 0, 18, 65) дают 100% покрытие ветвей.

Если добавить цикл, число путей бесконечно → 100% покрытие путей недостижимо.

Даже при полном покрытии ветвей возможны логические или граничные ошибки, поэтому покрытие — лишь один из критериев.
...
1. Тестирование системы в целом (Общее системное тестирование)
Тестирование системы в целом — это процесс проверки всей программной системы (включая аппаратное обеспечение, сеть, базы данных, внешние интерфейсы и т. д.) как единого целого в реальных или симулированных условиях эксплуатации. Основная цель — убедиться, что поведение всей системы полностью соответствует бизнес-требованиям и проектным спецификациям. 系统整体测试是指在真实或模拟的完整运行环境下，将整个软件系统（包括硬件、网络、数据库、外部接口等）作为一个整体进行验证的过程。它的核心目标是确保系统整体行为符合业务需求和设计规格

2. Системное тестирование (System Testing)
- Определение: Системное тестирование — это ключевой уровень тестирования в жизненном цикле ПО, выполняемый после интеграционного тестирования и перед приёмочным тестированием. Оно направлено на всестороннюю проверку функциональных и нефункциональных требований. 系统测试是软件测试生命周期中一个关键级别，发生在集成测试之后、验收测试之前。它主要针对功能性和非功能性需求进行全面验证。
- Среда выполнения: Обязательно проводится в отдельной тестовой среде, максимально приближенной к продуктивной. 必须在尽量接近生产环境的独立测试环境中执行。
- Основные области внимания:
  - Функциональность: Проверка корректности работы всех модулей согласно спецификации требований. 功能性：验证所有功能模块是否按需求规格说明书正确工作。
  - Нефункциональные аспекты: Включают производительность, безопасность, удобство использования, совместимость, надёжность и др. 非功能性：包括性能、安全性、可用性、兼容性、可靠性等。
- Основные методы: Преимущественно используется чёрный ящик (тестирование по входным и выходным данным без анализа внутреннего кода). Применяются методы эквивалентного разделения, анализа граничных значений, тестирование на основе сценариев использования и т.д. 黑盒测试为主（不关注内部代码逻辑，只关注输入和输出），常用技术包括等价类划分、边界值分析、用例测试等。

3. Тестирование производительности (Performance Testing)
Тестирование производительности — это наиболее важная и сложная часть нефункционального системного тестирования. Оно оценивает скорость, время отклика, стабильность, использование ресурсов и масштабируемость системы под определённой рабочей нагрузкой. Обычно делится на следующие подвиды: 性能测试是非功能性系统测试中最核心、最复杂的一类，用于评估系统在特定工作负载下的速度、响应时间、稳定性、资源利用率和可扩展性。它通常分为以下几个子类型：
- Нагрузочное тестирование (Load Testing) — моделирование ожидаемого количества одновременных пользователей для проверки достижения целевых показателей производительности. 负载测试
- Стресс-тестирование (Stress Testing) — постепенное увеличение нагрузки до точки отказа системы, чтобы определить её предельную пропускную способность и способность восстанавливаться после сбоев. 压力测试
- Тестирование стабильности / длительной нагрузки (Soak / Endurance Testing) — постоянная подача средней нагрузки в течение длительного времени (часы или сутки) для выявления утечек памяти, истощения ресурсов и других долгосрочных проблем. 耐久性/浸泡测试
- Тестирование пиковых нагрузок (Spike Testing) — моделирование резких, внезапных скачков трафика для проверки способности системы справляться с кратковременными всплесками запросов. 峰值测试
- Тестирование масштабируемости (Scalability Testing) — постепенное увеличение аппаратных ресурсов или количества пользователей для проверки линейности роста производительности системы. 可扩展性测试


## 64. Тестирование системы в целом. Альфа- и бета-тестирование.
1. Тестирование системы в целом

Определение: Это сквозное (E2E) тестирование всей системы (софт + железо + сеть + внешние интерфейсы) в среде, максимально приближенной к боевой.

Цель: Проверка не только функций, но и взаимодействия компонентов, общей производительности, безопасности и надёжности.

2. Альфа-тестирование (α-тестирование)

Кто проводит: Внутренняя команда (разработчики, тестировщики, продакт-менеджеры).

Где: В среде разработки или внутренней тестовой лаборатории (контролируемая среда).

Когда: Перед бета-тестированием.

Особенности: Имитирует действия пользователей, цель — быстро выявить грубые дефекты и логические ошибки.

3. Бета-тестирование (β-тестирование)

Кто проводит: Реальные конечные пользователи (внешние тестировщики или фокус-группа).

Где: В реальной боевой или предрелизной среде (неконтролируемая).

Когда: После альфа-тестирования, перед официальным релизом.

Особенности: Основной фокус — пользовательский опыт (UX), совместимость (разные устройства/браузеры) и сложные сценарии, которые не выявили внутренние тесты.

1. 整体系统测试（Тестирование системы в целом）

定义：在尽可能接近生产环境的完整环境中，对整个系统（软件+硬件+网络+外部接口）进行端到端（E2E）验证。

目标：不仅检查功能，还检查组件之间的交互、整体性能、安全性和可靠性。

2. Alpha测试（Альфа-тестирование）

执行方：内部团队（开发人员、测试工程师、产品经理）。

环境：开发环境或内部测试实验室（受控环境）。

时间：在Beta测试之前进行。

特点：模拟内部用户操作，目的是快速发现严重的缺陷和逻辑错误。

3. Beta测试（Бета-тестирование）

执行方：真实的最终用户（外部用户或种子用户）。

环境：真实的生产环境或预发布环境（非受控）。

时间：在Alpha测试之后、正式发布之前进行。

特点：重点验证用户体验（UX）、兼容性（不同设备/浏览器）以及内部测试未覆盖的复杂真实场景。
