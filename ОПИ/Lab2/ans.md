...
-X ours 或 --strategy-option=ours冲突时全部保留当前分支（ours）的内容，丢弃 incoming 分支的冲突部分git merge -X ours feature-X theirs 或 --strategy-option=theirs冲突时全部采用 incoming 分支（theirs）的内容，丢弃当前分支的git merge -X theirs main-X ignore-all-space忽略所有空白字符差异（非常有用，当冲突只是缩进/空格引起时）git merge -X ignore-all-space branch-X ignore-space-change忽略空格数量的变化（比上面稍宽松）git merge -X ignore-space-change branch  
...
在执行 r13 和 r14 的合并时，Git 报告了 Content Conflict 和 Add/Add Conflict。我通过提取目标版本的文件（对应 rs/r13 和 rs/r14）覆盖了工作区，并使用 git add 暂存，从而解决了冲突  
При слиянии r13 и r14 Git выдал сообщения о конфликтах содержимого и конфликтах добавления/добавления. Я разрешил конфликты, получив файлы из целевых версий (соответствующих rs/r13 и rs/r14), перезаписав их в рабочем каталоге, а затем добавив их в индекс с помощью команды `git add`.  

#### cherry pick
Команда `git cherry-pick` копирует один или несколько указанных коммитов из других веток, применяет их к текущей ветке и создает новый коммит в текущей ветке.  
git cherry-pick 把一个或多个指定的 commit（提交）从其他分支“复制”过来，应用到当前分支上，并在当前分支创建一个新的 commit。